...
Corti is publicly available and open source, licensed under the GNU GPLv3.  It is free to distribute and use by anyone.  The file format used to store large simulation output is the Hierarchial Data Format (HDF5), which is a database in a file that can be analyzed and accessed by any operating system and any modern programming language; while Corti is written in Python, analysis of its output may be done in \textsc{matlab} or R or SPSS or any other tool. 
